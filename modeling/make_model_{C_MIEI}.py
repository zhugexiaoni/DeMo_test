...
class DeMo(nn.Module):
    ...
    def _fused_logits(self, fr, fn, ft):
        """Compute fused logits using the SAME head as training (DIRECT=1 only)."""
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None, return_pattern=3, img_path=None):
        ...
        if self.training:
            ...
            if self.direct:
                # --- C-MIEI intervention happens here (DIRECT path) ---
                enable = False
